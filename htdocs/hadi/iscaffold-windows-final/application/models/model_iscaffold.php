...
*/         		
		foreach ( $fields as $key => $field )
		{
            ...
            else if( $this->table_config[$key]['sf_type'] == 'many_related' )
            {
		        //$fields_array  .= ( $this->table_config[$key]['sf_type'] == 'many_related' ) ? '' : $separator . "\n\t'" . $field . "' => lang('" . $field . "')";
            }
            else
            {
			    $fields_string .= $separator . $this->table_config[$key]['sf_table']. '.' . $field;
            }
            // SKIP 'Virtual fields'
            $raw_fields_string .= ( $this->table_config[$key]['sf_type'] == 'many_related' ) ? '' : $separator . $this->table_config[$key]['sf_table']. '.' . $field;
		    $fields_array      .= $separator . "\n\t'"  . $this->table_config[$key]['sf_table']. '.' .$field . "' => lang('" . $this->table_config[$key]['sf_table']. '.' . $field . "')";
		}
			...
			else
			{
                ...
                if( isset( $table_meta[ $field ] ) )
                {
                    if( $table_meta[ $field ]['null'] == 'no' )
                    {
                        ...
                        // Don't validate for 'virtual' fiels
                        if( $this->table_config[$key]['sf_type'] !== 'many_related' )
                        {
        				    $set_rules .= "\$this->form_validation->set_rules( '$field', lang('$field'), '" . implode( '|', $rules ) . "' );\n";
                        }
                    }
                    else
                    {
                        ...
                        {
                            $ml = 'max_length['.$table_meta[ $field ]['max_length'].']';
    				        $set_rules	.= 	"\$this->form_validation->set_rules( '$field', lang('$field'), '$ml' );\n";
                        }
                    }
                }
                ...
                {
                    case 'checkbox':
    				    $validation_true .=	"\$data_post['$field'] = ( \$this->input->post( '$field' ) == FALSE ) ? $this->checkbox_default : \$this->input->post( '$field' );\n";
                    break;

                    case 'date':
    				    $validation_true .=	"\$data_post['$field'] = strtotime( \$this->input->post( '$field' ) );\n";
                    break;

                    case 'file':
    				    $validation_true .=	"\$data_post['$field'] = ( empty( \$_FILES['$field']['name'] ) ) ? \$this->input->post( '$field-original-name' ) : \$this->uploader->upload( '$field' );\n";
    				break;
                    ...
                    default:
    				    $validation_true .=	"\$data_post['$field'] = \$this->input->post( '$field' );\n";
    				break;
                }
                ...
                if( $this->table_config[$key]['sf_type'] == 'related' )
                {
                    ...
                    **/                                                                                 
                    ...
                    {
                        $model_joins    .= "\$this->db->join( '$rel_data[0]', '{$this->table_config[$key]['sf_table']}.$field = $rel_data[0].$rel_data[1]', 'left' );\n";
                        ...
                        $model_related  .= "\tfunction related_$rel_data[0]()
    {
		...
		**/
        ...
        $file_upload_validation = ( !$has_file_field ) ? '' : '|| $this->uploader->success == FALSE ';
        $load_upload_model      = ( !$has_file_field ) ? '' : "\$this->load->model( 'uploader' );";
        $file_upload_errors     = ( !$has_file_field ) ? '' : "\$errors .= \$this->uploader->response;";
        ...
        **/
        ...
        /**
            Generate Frame_public.tpl VIEW templates
            Right now it is a static file, nothing to configure
        **/

        $frame_view_path = $data_path['output_view'] . 'frame_public.tpl';
        ...
        if( !file_exists( $frame_view_path ) )
        {
            ...
            {
                if( $t !== 'sf_config' ) $list_of_tables .= "<li>&diams; <a href='$t'>".ucfirst($t)."</a></li>\n";
            }
            $list_of_tables .= '</ul>';

            $frame_view_content = read_file( $data_path['input_view'] . 'frame_public.tpl' );
