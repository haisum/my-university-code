...
<html>
    ...
    <head>
        ...
        <title>Wedding Price Sample - Faq</title>
    ...
    <p><a href="<?php echo  URL;?>/contact-support.php" class="support-button">Contact Support</a></p>
        ...
        <div id="rightCol" class='rightCol'>
			...
			<?php 
				...
				require_once 'classes/database/objects/class.faqcategory.php';
				require_once 'classes/database/objects/class.faq.php';
				$faqCat = new FaqCategory();
				$faqCatList = $faqCat->GetList(array(), 'position', true, '');
				foreach($faqCatList as $key => $faq){
				$style = "";
				if(($key+1)%4 ==0)
					$style="style='clear:left;'";
			?>
				<div class="helpColumn" <?php echo $style;?>><h3><a href="<?php echo URL. '/faq-detail.php?category=' . $faq->faqcategoryId;?>" class="more"><?php echo $faq->title;?></a></h3>	
					<ul>
						<?php 
							$faqList = $faq->GetFaqList(array(), 'position', true, 5);
							foreach($faqList as $faqQuestion){
						?>
							<li><a href="<?php echo URL. '/faq-detail.php?category=' . $faq->faqcategoryId  . '#' . $faqQuestion->faqId; ?>"><?php echo $faqQuestion->question; ?></a></li>
						<?php } ?>
						<a href="<?php echo URL. '/faq-detail.php?category=' . $faq->faqcategoryId;?>" class="more">More..</a>
					</ul>
				</div>
			<?php
				}
