...
$limit = 20; 								//how many items to show per page
	...
	*/
	...
	$targetpage = "main-faq-listing.php"; 	//your file name  (the name of this file)
	$page = $_GET['page'];
	if($page) 
...
<script>
function delete_(faqid)
{
	var confirm_value = confirm('Are you sure you want to delete this FAQ? This action is IR-REVERSIBLE.');
	
	if(confirm_value == true)
	{
		window.location.href ="delete.php?faqid="+faqid;
	}
}
...
?>
						    <tr>
							  <th width="40"><?=trimlong($row['question'])?></th>							  
							  <th width="40"><?=trimlong($row['answer'])?></th>							  
							  <th width="40"><?=$row['position']?></th>	
							  <th width="40">
								<?=select(getFaqcategories(),  $row['faqcategoryid'], 'faqCategory',  'faqCategory' . $row['faqid'], '', 
							  ' onchange="ajaxUpdate(\'faq\', \'faqcategoryid\', $(\'#faqCategory' .$row['faqid'].'>option:selected\').val(), \'faqid\', \'' .$row['faqid'].'\')"')?>
							  </th>
							  <td width="20">
									<a href="add-edit-faq.php?faqid=<?=$row['faqid']?>"><img src="images/icon_edit.png" alt="edit" class="help" title="Edit"/></a>
									&nbsp;&nbsp;
									<a href="javascript:void(0);"><img src="images/icon_delete.png" alt="Delete" class="help" title="Delete Row" onclick="delete_('<?=$row['faqid']?>'); return false;" /></a>   									
							  </td>
						    </tr>
                <?
              }
