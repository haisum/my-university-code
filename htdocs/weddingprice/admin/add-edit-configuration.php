...
$isEdit = false;
$row = array();
if(isset($_REQUEST['configurationid'])){
	$configurationid = intval($_REQUEST['configurationid']);
	$query = mysql_query("SELECT configuration.* from configuration where configurationid=$configurationid");
	...
	$('#edit_email_template').click(function() 
	{
		...
		if(error >=1)
		{
			$.blockUI({
				theme:     false,
				title:    'Error',
				configuration:  '<strong><font color=red><p><br>Fields marked with (*) are compulsory.<br><br></p></font</strong>',
				timeout:   2000
			});
...
<!-- End 2nd level tab -->All fields marked with (<strong>*</strong>) are compulsory</div>
				...
				<div class="content nomargin">
					...
					?>
                    	...
                    	<input type="hidden" name="configurationid" value="<?=$_GET['configurationid'];?>" />
					...
					?>                    
					  ...
					  <label>Value:</label>
					    *<br/>
					    <input type='text' name='value' <?php if(is_numeric($row['value'])){ echo 'class="numberOnly"';} ?> value="<?=$row['value']?>" id='value' />
                        <span id="name-error"></span>
					  <br/>	
					  <br/>	
