...
$isEdit = false;
$row = array();
if(isset($_REQUEST['faqcategoryid'])){
	$query = mysql_query('SELECT * from faqcategory where faqcategoryid=' . intval($_REQUEST['faqcategoryid']) . ' LIMIT 1');
...
<!-- End 2nd level tab -->All fields marked with (<strong>*</strong>) are compulsory</div>
				...
				<div class="content nomargin">
					...
					?>
                    	...
                    	<input type="hidden" name="id" value="<?=$row['faqcategoryid'];?>" />
					...
					?>
					  ...
					  <!-- Begin form elements -->
					    <label>Title:</label>
					    *<br/>
					    <textarea class="text" id="title"  name="title"><? if($isEdit) echo $row['title']; ?></textarea>
					  ...
					  <input type="submit" id="edit_email_template" class="button_dark" value="<? if($isEdit) echo "Edit Category"; else echo "Add Category"; ?>"/>
