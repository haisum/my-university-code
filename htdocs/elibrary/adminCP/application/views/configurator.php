...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		<script src="<?php echo base_url(); ?>application/js/mootools.js"></script>
		<script src="<?php echo base_url(); ?>application/js/mootools-more.js"></script>
		<script src="<?php echo base_url(); ?>application/js/configurator.js"></script>
		...
		<div id="container">
			...
			<div id="content">
				<div id="welcome">
                    ...
                    <?php if( !empty( $config ) ): ?>
    					<p id="conf-menu">
                            <a href="javascript:conf.check( 'configurator/modify/reset', 'Are you sure? This action will erase all the settings you made.' )" id="" class="link-icon">Reset config table</a> delete all settings you made, but keep the config table's fields.<br />
                            <a href="javascript:conf.check( 'configurator/modify/generate', 'Are you sure? This action will drop the config table.')" id="" class="link-icon">Regenerate config table</a> truncates config table and refills it from your database schema.<br />
                            <a href="javascript:conf.check( 'configurator/modify/update', 'Are you sure? This action will modify the settings you made.')" id="" class="link-icon">Update config table</a> scans your schema, if you made changes, like a new table or field, it will be added to the config table.<br />
                        </p>
                    <?php endif; ?>
                    ...
                    <?php else: ?>
                        <div id="warning-message" class="message">
                            <p>Database table "sf_config" does not exists, <a href="configurator/create_table" class="link-icon">click here to create it.</a></p>
    					</div>
                        ...
                        regenerate the source codes more than on times.
                        <br />
                        <br />
                    <?php endif; ?>
                    ...
                    <?php if( !empty( $config ) ): ?>
                                    ...
                                    <?php foreach( $tdata['fields'] as $key => $field ): ?>
                                      <li>
                                        ...
                                        <div class="field">
                    					   ...
                    					   <label for="h_<?=$table?>-<?=$field?>">Hidden</label>
                    					   <input type="checkbox" disabled="disabled" name="hidden" id="h_<?=$table?>-<?=$field?>" <?php if( $tdata['data'][$key]['sf_hidden'] == 1 ): ?>checked="checked"<?php endif ?>/><br />
                                        </div>
                                      </li>
                                    <?php endforeach; ?>
                                </ul>
                        ...
                        <a href="<?php echo base_url();?>" class="button negative">Back WITHOUT saving</a>
					<?php endif; ?>
                    </div>
				</div>
			</div>
